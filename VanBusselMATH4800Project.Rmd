...
This section will provide a "crash course" in time series. This background is necessary in order to understand the context as well as the significance of the goals of this Honours Project.
...
Instead, it is useful to scale the covariance of the two variables by their standard deviations. This is equivalent to calculation the *correlation* between the two variables. This is a more interpetable value -- recall that correlations can range from -1 to +1, where -1 represents a perfectly negative linear relationship, +1 represents a perfectly positive linear relationship, and 0 represents no linear assocation. 
...
Stationary processes are much easier to model, as they have less parameters. The estimators for these processes are quite straightforward as well. For example, the estimator the mean, $\mu$, is simply the sample average: that is, $\hat{\mu} = \bar{y}$. 
...
The model procduces the following estimates for the series: 
...
Consider the simplest Moving Average (MA): an MA(1) process. A time series $X_1, X_2, ...$ is said to be a *simple moving average process* iff: 

$$
X_t = \mu + \epsilon_t + \theta \epsilon_{t-1}
$$

where $\epsilon_t \sim \text{WN}(\mu, \sigma_{\epsilon}^2)$. This is quite similar, conceptually, to the AR(1) process, except that the current observation is regressed on the noise of the previous observation. Again, this process has 3 paramters. This time, the slope coefficent is represented by $\theta$. And, again, if the slope coefficient is zero, then the series $X_t$ is a white noise process. And, again...larger values of the slope coefficient correpond to higher autocorrelation, while negative values of $\theta$ correpond to series with periodic behavior.
...
This is just one possible reason why a time series may have missing values. Another common reason that this occurs is because of equipment failure. It is also possible to have time series in which some observations need to be removed due to transcription error or unreliable results. 
...
In this section, some interpolation algorithms will be presented. If they are not already available for use in R, they will be implemented in R.
...
It is important to note that, while some of these algorithms are very simplistic, even the simplest of algorithms (Nearest-Neighbor) are present in the literature as recently as 2014. This demonstrates that this is still a very active field of research, and that there is much work to be done in this area.
...
This algorithm was first presented (large scale) in 1980. While it may seem very simple, all of these algorithms are relatively "new", as these sorts of algorithms were not viable on a large scale before the invention of computers.
...
It might seem like this is harmless, but a rather shocking fact is that when you Google search for "Time Series Interpolation in R", the first link that appears is a Stack Overflow question in which the person is told to use the `na.approx` function to interpolate the missing points in their series. It is alarming that such a simplistic method is being used on such a large scale. 

Another member of the `na.approx` family of functions from the `zoo` package in R is the `na.spline` function. This function uses cubic spline interpolation. 
...
Note that these cubic spline interpolation methods are also presented in the Lepot paper.
...
Kalman filters are used widely in other scientific fields. For example, they can be used for robotic motion planning and control, as they can be used to model the way that the central nervous system controls our movement.
...
One of the existing problems in this field of research is that the criteria used for evaluating the performance of interpolation algorithms has been extremely inconsistent. This may be due to personal opinions of authors, or perhaps it is because authors choose to use whichever criteria will best support their research -- "cherry picking". Another problem that has arisen is that authors in the literature seem to be using different names for the same criteria, perhaps out of personal preference or perhaps in an attempt to sound overly sophisticated. To combat these inconsistencies, it will be useful to have a function which will allow a user to input the original, unaltered and complete time series (let this be known as $x$), and the interpolated series after gaps have been imposed and filled (let the interpolated series be known as $X$). This function should then output a number of performance criteria. 
...
However, upon using this formula on real data, I realized that this formula is actually incorrect. The formula *should* have been presented in Table 1 as: 

$$
r = \frac{\sum_{i = 1}^n(X_i - \bar{X})(x_i - \bar{x})}{\sqrt{\sum_{i=1}^n(X_i - \bar{X})^2} \sqrt{\sum_{i=1}^n(x_i - \bar{x})^2}}
$$ 

which is significantly different from the formula presented in the mentioned paper. The correct formula forces values to be between -1 and +1. 
...
The formula presented in Table 1 of the paper is not represented in a mathematically correct way. The correct formula is:
...
The Mean Squares Error (denoted NMSE) is not really what it sounds like. It appears to have only been used in one paper. Its definition is:
...
The code for this analysis is far too long to include in this report, but the following general approach was taken for each obtained time series: 
...
The third and most complex dataset that was used for this project is one which is readily available in Wesley Burr's `tsinterp` package, entitled `flux`. This dataset contains daily measurements of solar flux. *Solar flux* is a measure of how much light energy is being radiated in a given area, and in this dataset it is measured in solar flux units (SFU). 
...
This entire experiment was only run once, with one seed value. It would be ideal to run this experiment 1000s of times to obtain better results, but this would require significant computational power.

### Increase and refine algorithms and criteria

An obvious extension would be to add more algorithms and performance criteria to the experiment. A great starting place would be to add a "calculation time" criteria, which would favour algorithms that are fast. 
...
1. https://stackoverflow.com/questions/44008828/r-markdown-format-title-pdf-output

2. https://stackoverflow.com/questions/30972442/how-to-separate-title-page-and-table-of-content-page-from-knitr-rmarkdown-pdf

3. http://www.brightsourceenergy.com/understanding-solar-flux-science-vs-science-fiction-1

4. https://haozhu233.github.io/kableExtra/awesome_table_in_pdf.pdf

5. https://stackoverflow.com/questions/12995683/any-way-to-make-plot-points-in-scatterplot-more-transparent-in-r

6. https://www.analyticsvidhya.com/blog/2015/12/complete-tutorial-time-series-modeling/

7. https://tex.stackexchange.com/questions/32140/how-to-write-a-function-piecewise-with-bracket-outside

8. https://stackoverflow.com/questions/14450384/create-a-vector-of-all-days-between-two-dates

9. https://en.wikipedia.org/wiki/Kalman_filter

10. https://en.wikiversity.org/wiki/Cubic_Spline_Interpolation
